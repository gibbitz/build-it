...
var config          = require('./gulp.paths.json'),
    ...
    vendorFontGlob  = [
                        base.npm + 'owfont/fonts/owfont-regular.*'
                      ],
    cleanGlob       = [
                        base.tmp + '**',
                        '!' + base.tmp,
...
// ||||||||||||||||||||||||||||||||||||||||||||||||||||
  ...
  gulp.task('copyFonts', ['clean'], function(){
    return gulp.src(vendorFontGlob)
      .pipe(plugins.plumber())
      .pipe(plugins.flatten())
      .pipe(gulp.dest( base.tmp + paths.fonts ));
  });

  gulp.task('copyFonts:release', ['clean'], function(){
    return gulp.src(vendorFontGlob)
      .pipe(plugins.plumber())
      .pipe(plugins.flatten())
  ...
  gulp.task('modifyFontpaths', ['prebundle'], function(){
    return gulp.src(base.tmp + paths.scss + '**')
      .pipe(plugins.plumber())
      .pipe(plugins.replace('../fonts', '/ui/fonts'))
      .pipe(gulp.dest( base.tmp + paths.scss ));
  });
